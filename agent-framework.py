...
class ClaudeAgent:
    ...
    def call_claude(self, user_message: str, temperature: float = 0.7, max_tokens: int = 4000) -> str:
        ...
        """
        # For demo mode, just return a basic response
    ...
    def _generate_mock_response(self, user_message: str) -> str:
        ...
        if "keyword" in self.name:
            mock_response = json.dumps({
                "analysis": f"Analyzed target keywords for {self.name}",
                "recommendations": [
        ...
        elif "content" in self.name:
            mock_response = json.dumps({
                "analysis": f"Analyzed content structure and opportunities for {self.name}",
                "recommendations": [
                    "Add more detailed headings to improve structure",
                    "Include more visual elements like images and videos",
                    "Expand sections on competitor analysis"
                ],
                "content_score": 78
            })
        elif "technical" in self.name:
            mock_response = json.dumps({
                "analysis": f"Performed technical SEO audit for {self.name}",
                "recommendations": [
                    "Improve page load speed by optimizing images",
                    "Fix broken links on product pages",
        ...
        elif "strategy" in self.name:
            mock_response = json.dumps({
                "analysis": f"Developed comprehensive SEO strategy for {self.name}",
                "recommendations": [
                    "Prioritize content creation for high-value keywords",
                    "Build quality backlinks from industry publications",
                    "Implement schema markup for better SERP features"
                ],
                "priority_actions": ["fix technical issues", "optimize top pages", "create content calendar"]
            })
        else:
            mock_response = json.dumps({
                "analysis": f"Processed input for {self.name}",
                "recommendations": [
